...
(
    ...
    image_url                        varchar(2083)                        not null comment 'URL to the image of the connector logo',
    ...
    is_enabled                      tinyint(1) default 1                 not null comment 'Set to 1 if the connector should be returned when listing connectors',
    is_oauth                        tinyint(1) default 0                 not null comment 'Set to 1 if the connector uses OAuth authentication as opposed to username/password',
    ...
    constraint connectors_slug_uindex
        unique (slug),
    constraint connectors_oauth_client_id_fk
        foreign key (oauth_client_id) references oauth_clients (id)
)
    comment 'A connector pulls data from other data providers using their API or a screen scraper. Returns a list of all available connectors and information about them such as their id, name, whether the user has provided access, logo url, connection instructions, and the update history.'
    charset = utf8;
